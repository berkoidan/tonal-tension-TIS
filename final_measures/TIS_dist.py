...
def TIS_dist(c1, c2, vkey, tf):
    ...
    tkey, mod_key = normal_fft(midi2chroma(vkey[0]))
    v3 = key_dist(t2, tkey)
    
    # d) Compare distance between the chord and the harmonic functions
    tokey, mod_t = normal_fft(midi2chroma(vkey[1]))
    dokey, mod_d = normal_fft(midi2chroma(vkey[2]))
    sdkey, mod_s = normal_fft(midi2chroma(vkey[3]))
