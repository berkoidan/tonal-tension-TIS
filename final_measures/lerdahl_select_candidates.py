...
def lerdahl_select_candidates(t, chords, seq, skey):
    ...
    skey = list(map(lambda key: key % 12, skey))
    # Normalization
    # max_d = 13
    # max_vl = 15.73
    # max_c = 7
    ...
    v[0, 1], v[0, 4], v[0, 5] = lerdahl_dist(chords[0], chords[0], skey)
    v[0, 3] = lerdahl_dissonance(chords[0], skey)
    for i in range(1, len(chords)):
        inversion, v[i, 0] = lerdahl_voices2(chords[i - 1], chords[i], skey)
        chords[i] = inversion
        # Harmonic attraction rule: h=c*voice_leading/d, where c=10
        # (maximization function)
        v[i, 1], v[i, 4], v[i, 5] = lerdahl_dist(chords[i - 1], chords[i], skey)
        dist_final = (v[i, 1] + v[i, 4] + v[i, 5])
        if dist_final == 0:
            v[i, 0] = 0
        else:
            v[i, 0] = v[i, 0] / dist_final
        v[i, 3] = lerdahl_dissonance(chords[i], skey)
    # v[:, 2] = lerdahl_global_surface(t, chords, seq, skey)
    # print(lerdahl_global_surface(t, chords, seq, skey))
    print(chords, v)
    return chords, v
